-- 1. Habilitar a extensão HTTP (Permite que o banco acesse a internet)
CREATE EXTENSION IF NOT EXISTS http WITH SCHEMA extensions;

-- 2. Criar a Função Segura de Envio (Roda no Servidor)
...
BEGIN
  ...
  VALUES (target_id::text, title, body, now());
  ...
  IF v_resp_status = 200 THEN
     -- Expo retornou 200, mas precisamos ver se deu "ok" ou "error" dentro do JSON
     RETURN json_build_object(
       'success', true, 
...
-- 3. Liberar acesso para o Painel Web chamar essa função
GRANT EXECUTE ON FUNCTION send_push_notification_rpc(UUID, TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION send_push_notification_rpc(UUID, TEXT, TEXT) TO anon;
