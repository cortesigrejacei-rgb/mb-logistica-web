...
export const Mapa = () => {
  ...
  useEffect(() => {
    ...
    if (mapInstance.current && !techLayerRef.current) {
      techLayerRef.current = L.layerGroup().addTo(mapInstance.current);
    }
  }, [mapInstance.current]);
  ...
  useEffect(() => {
    const syncAllRoutes = async () => {
      ...
      for (const tech of technicians) {
        // Calculate total planned distance using ALL of today's tasks (plus any straggler pending tasks)
        const allTechTasksToday = collections
        ...
        if (allTechTasksToday.length === 0) continue;

        // Start/End logic (CRITICAL: Never fallback to tech.lat/lng moving GPS for the Total Route Origin)
        const startLat = tech.start_lat || -26.897244;
        const startLng = tech.start_lng || -49.065570;
        if (!startLat || !startLng) continue;

        const coords = [`${startLng},${startLat}`];
        allTechTasksToday.forEach(c => {
        ...
        try {
          ...
          if (json.code === 'Ok' && json.routes[0]) {
            ...
            const { error } = await supabase.from('route_summaries').upsert({
              ...
              collection_count: allTechTasksToday.length,
  ...
  useEffect(() => {
    ...
    const fetchRoute = async () => {
      // The visual polyline should start from the technician's planned Base, NOT their live location
      ...
      const coords = [`${startLng},${startLat}`]; // Start exactly at the base
      techCollections.forEach(c => {
        if (c.lat && c.lng) coords.push(`${c.lng},${c.lat}`);
      });

      // Destination is either end point, base, or back to self
      const endLat = tech.end_lat || tech.start_lat || startLat;
      const endLng = tech.end_lng || tech.start_lng || startLng;
      coords.push(`${endLng},${endLat}`);
      ...
      try {
        ...
        if (json.code === 'Ok' && json.routes[0]) {
          ...
          // Just display the remaining polyline. We no longer save this partial distance to the database,
          // because it would overwrite the TRUE total distance calculated by syncAllRoutes.
              ...
              {technicians.map(tech => (
                <TechListItem
                  ...
                  route={selectedTechId === tech.id && routeStats
                    ...
                    : (tech.remaining_km !== undefined && tech.remaining_km !== null) && tech.status === 'Em Rota' ? `Faltam: ${tech.remaining_km.toFixed(1)} km` : tech.status}
                  status={tech.status}
                  statusColor={
