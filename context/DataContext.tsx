...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const mapCollectionFromDB = (dbCol: any): Collection => {
    ...
    return {
      ...
      updatedAt: dbCol.updated_at,
      ...coords
    };
  };

  const fetchData = async () => {
    // Safety timeout to prevent infinite loading in case of network/RLS hang
    const timeout = setTimeout(() => {
      if (loading) {
        console.warn("[DataContext] fetchData took > 10s, forcing loading off.");
        setLoading(false);
      }
    }, 10000);
    ...
    try {
      ...
      const { data: cols, error: colError } = await supabase.from('collections').select('*').order('date', { ascending: false });
      if (colError) throw colError;
      if (cols) {
        setCollections(cols.map(mapCollectionFromDB));
      }

      const { data: stock, error: stockError } = await supabase.from('stock_items').select('*, proof_url, notes');
      ...
      const { data: sets, error: settingsError } = await supabase.from('settings').select('*').single();
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('app_sync')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'technicians' }, (payload) => {
        console.log('[Web Sync] Technician update:', payload);
